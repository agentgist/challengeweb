...
export const HomeView: React.FC<HomeViewProps> = ({
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8 sm:py-16 space-y-10 text-center" dir="rtl">
      
      {/* Centered Brand Header - Google Style */}
      <div className="space-y-3 select-none">
        <div className="w-14 h-14 mx-auto rounded-2xl bg-slate-900 text-amber-400 flex items-center justify-center shadow-md">
        ...
        <p className="text-xs sm:text-sm text-slate-500 font-medium max-w-sm mx-auto">
          منصة المسابقات المدرسية والشهادات الفورية
        </p>
      </div>

      {/* The Two Main Options: Student vs Teacher */}
      <div className="inline-flex p-1.5 bg-slate-200/70 rounded-2xl border border-slate-300/60 shadow-2xs">
        <button
          onClick={() => {
            setActiveRole('student');
            setCodeError(null);
          }}
          className={`px-6 sm:px-8 py-2.5 rounded-xl font-black text-xs sm:text-sm transition-all cursor-pointer flex items-center gap-2 ${
            activeRole === 'student'
        ...
        <div className="space-y-8 animate-in fade-in duration-200 text-right">
          ...
          <div className="max-w-2xl mx-auto">
            <div className="relative">
              <input
                ...
                className="w-full pr-11 pl-4 py-3.5 bg-white border border-slate-200/90 rounded-full text-xs sm:text-sm font-bold text-slate-800 shadow-sm focus:outline-slate-900 focus:ring-2 focus:ring-slate-900/10 transition-all placeholder:text-slate-400 placeholder:font-normal"
              />
              <Search className="w-4 h-4 text-slate-400 absolute right-4 top-4" />
              {searchQuery && (
                <button
                  onClick={() => setSearchQuery('')}
                  className="absolute left-4 top-3.5 text-slate-400 hover:text-slate-700 text-xs font-bold"
                >
                  مسح
                </button>
              )}
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between text-xs text-slate-500 font-bold px-1">
              ...
              <span>اختر مسابقة للبدء فوراً</span>
            </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {filteredCompetitions.map((comp) => {
                  ...
                  return (
                    <div
                      key={comp.id}
                      className="bg-white rounded-2xl border border-slate-200/80 p-5 shadow-2xs hover:shadow-md transition-all flex flex-col justify-between space-y-4 hover:border-slate-300"
                    >
                      <div className="space-y-2.5">
                        ...
                        <div className="flex flex-wrap items-center gap-2 text-[11px] text-slate-500 pt-0.5">
                          ...
                          {comp.singleAttempt && (
                            <>
                              <span>•</span>
                              <span className="text-rose-700 font-bold">محاولة واحدة</span>
                            </>
                          )}
                        </div>
                        ...
                        {/* Timing Note */}
                        {comp.competitionType === 'live' && (
                          <div className="text-[10px] text-slate-500 bg-slate-50 p-2 rounded-xl border border-slate-200/60">
          ...
          {currentTeacher ? (
            ...
            <div className="bg-white rounded-3xl border border-slate-200 p-6 sm:p-8 text-center space-y-4 shadow-sm">
              ...
              <button
                ...
                className="w-full py-3 bg-slate-900 hover:bg-slate-800 text-white rounded-xl text-xs font-black shadow-md cursor-pointer transition-all flex items-center justify-center gap-2"
              >
            ...
            <div className="bg-white rounded-3xl border border-slate-200 p-6 sm:p-8 text-right space-y-5 shadow-sm">
              <div className="text-center space-y-1">
                <div className="w-12 h-12 rounded-2xl bg-slate-100 text-slate-800 flex items-center justify-center mx-auto mb-2">
                ...
                <p className="text-xs text-slate-500">
                  لإنشاء مسابقات لطلابك ومتابعة النتائج وطباعة الشهادات
                </p>
              </div>
              ...
              <form onSubmit={handleTeacherCodeSubmit} className="space-y-3 pt-1">
                <div>
                  <input
                    ...
                    className="w-full text-center tracking-widest font-mono text-sm sm:text-base font-black px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-slate-900 focus:outline-slate-900 focus:bg-white transition-all uppercase"
                  />
                  {codeError && (
                    <div className="flex items-center gap-1 text-xs text-rose-600 font-bold mt-2 pr-1">
                ...
                <button
                  ...
                  className="w-full py-3 bg-slate-900 hover:bg-slate-800 text-white rounded-xl text-xs sm:text-sm font-black shadow-md cursor-pointer transition-all active:scale-98 disabled:opacity-50"
                >
              ...
              <div className="pt-3 border-t border-slate-100 text-center">
                <button
                  ...
                  className="text-xs text-slate-500 hover:text-slate-900 font-bold cursor-pointer transition-colors"
                >
